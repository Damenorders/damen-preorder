...
(function() {
  ...
  function acEnsureVisible() {
    ...
    // Measure the LIST, do not assume a fixed gap below the field. The list is
    // mounted after the whole item row, so on the mobile card layout it starts a
    // card-height below the input — a fixed "is there 159px under the field?"
    // test passed while most of the list was still under the keyboard.
    const listOpen = !!(acEl && acEl.isConnected && acEl.style.display !== 'none');
    const lr = listOpen ? acEl.getBoundingClientRect() : null;
    if (r.top >= v.top + AC_MARGIN && lr && lr.bottom <= v.bottom) return;
    if (r.top >= v.top + AC_MARGIN && !lr &&
        (v.bottom - r.bottom) >= Math.min(220, Math.round(v.h * 0.35))) return;
  ...
  // Keep --rl-vvh / --rl-vvtop in step with the VISUAL viewport so a fixed
  // overlay covers exactly what is on screen. With a phone keyboard open the
  // layout viewport (and so vh / 100%) still reports the full page height,
  // which is what left the bottom of the pallet editor behind the keyboard.
  function syncViewportVars() {
    const vv = window.visualViewport;
    const h = (vv && vv.height) || window.innerHeight || 0;
    // A hidden/background tab can report 0 for a frame. Writing that through
    // would collapse the overlay to nothing, so keep the last good value.
    if (!h) return;
    const el = document.documentElement;
    el.style.setProperty('--rl-vvh', h + 'px');
    el.style.setProperty('--rl-vvtop', (vv ? vv.offsetTop : 0) + 'px');
  }
  function setupViewportVars() {
    if (window.__rlVvSetup) return;
    window.__rlVvSetup = true;
    syncViewportVars();
  ...
  function renderOverviewDry() {
    const vaisle = (label, floorId, grow, extraClass) => {
      const count = floorId ? floorItems(floorId).length : 0;
      const cls = 'rl-vaisle' + (floorId ? ' rl-vaisle-clickable' : '') + (extraClass ? ' ' + extraClass : '');
      const clickAttr = floorId ? `class="${cls}" onclick="RL.openFloorEditor(${floorId})" title="${esc(label)} — click to log pallets stored on the floor here"` : `class="${cls}"`;
      return `
    ...
    const mainHtml = runWrap(getRow(17), 'v', 'flex:0 0 80px; min-height:480px;', 480) + floor3Group + pair(15, 13, null, 'thin', 'thin') + vaisle('Floor 2', 2, 0.9, 'rl-floorshift') + pair(12, 11, 'wall', 'thin', 'thin')
      + vaisle('Floor 1', 1, 0.9, 'rl-floorshift') + runWrap(getRow(10), 'v', 'flex:0 0 80px; min-height:480px;', 480) + wall + rack5Corner;
      ...
      itemsHtml += `
        <tr class="rl-itemrow">
          ...
          <td class="rl-actcol" style="border:none; padding:0 0 0 4px; vertical-align:middle; white-space:nowrap;">
            ...
            <button class="rl-rowbtn" onclick="RL.removeItem(${i})"
    ...
    return `
      <div class="rl-overlay" onclick="if(event.target===this) return;">
        <div class="rl-modal">
          ...
          <table class="rl-itemtable" style="width:100%; border-collapse:collapse; table-layout:fixed;">
            <thead>
              <tr>
      ...
      itemsHtml += `
        <tr class="rl-itemrow">
          ...
          <td class="rl-actcol" style="border:none; padding:0 0 0 4px; vertical-align:middle; white-space:nowrap;">
            ...
            <button class="rl-rowbtn" onclick="RL.removeFloorItem(${i})"
  ...
  // Reflect a catalog auto-fill back into a single item row WITHOUT rebuilding
  // the modal. The full render() that used to run here replaced the DOM under the
  // user's finger: it dropped focus (so the keyboard shut after every pick),
  // reset the modal scroll position to the top, and swallowed taps on the row's
  // own move/remove buttons — the blur-driven change fired first and destroyed
  // the button before its click could land.
  function patchItemRow(editor, i, it) {
    const inputs = root.querySelectorAll(
      '.rl-cat-input[data-editor="' + editor + '"][data-index="' + i + '"]');
    if (inputs.length < 2) { render(); return; }
    const desc = inputs[0], sku = inputs[1];
    if (desc.value !== (it.description || '')) desc.value = it.description || '';
    if (sku.value !== (it.sku || '')) sku.value = it.sku || '';
    ...
    if (needWarn && !warn) {
      ...
      d.textContent = 'Not in the item catalog';
      cell.appendChild(d);
